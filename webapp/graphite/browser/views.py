...
def browser(request):
    ...
    if context['target']:
        # js libs terminate a querystring on #
        context['target'] = context['target'].replace('#', '%23')
...
def myGraphLookup(request):
    ...
    try:
        ...
        matches = [graph for graph in profile.mygraph_set.all().order_by(
            'name') if graph.name.startswith(userpath_prefix)]

        log.info(
            'myGraphLookup: username=%s, path=%s, userpath_prefix=%s, %ld '
            'graph to process' % (profile.user.username, path,
                                  userpath_prefix, len(matches)))
        branch_inserted = set()
        leaf_inserted = set()
        ...
        if not username:
            ...
            for profile in profiles:
                if profile.mygraph_set.count():
                    node = {'text': str(profile.user.username), 'id': str(
            ...
            matches = [graph for graph in profile.mygraph_set.all().order_by(
                'name') if graph.name.startswith(prefix)]
            inserted = set()
